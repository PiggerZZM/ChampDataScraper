...
def run(years: List[str], matches: List[str]):
    # 获取所有比赛详情页url
    ...
    for match_url in match_urls:
        ...
        for half in [0, 1, 2]:
            # 请求"球队数据"
            json_data_list_static.append(get_json_data(ajax_url_static, match_id, half, l_team, r_team))
            json_data_list_attack.append(get_json_data(ajax_url_attack, match_id, half, l_team, r_team))
            json_data_list_defence.append(get_json_data(ajax_url_defence, match_id, half, l_team, r_team))
    # 存储
    json_to_csv(json_data_list_static, file_name=r"match_data_static_{}-{}.csv".format(years[0], years[-1]))
    json_to_csv(json_data_list_attack, file_name=r"match_data_attack_{}-{}.csv".format(years[0], years[-1]))
    json_to_csv(json_data_list_defence, file_name=r"match_data_defence_{}-{}.csv".format(years[0], years[-1]))
